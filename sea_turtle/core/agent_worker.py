...
class AgentWorker:
    ...
    async def _run_job_step(self, msg: dict) -> dict[str, Any]:
        ...
        user_message = (
            ...
            "First infer the most appropriate final deliverable for this job: code changes, a document/file, or another concrete artifact.\n"
            "Do not default to writing a markdown report if the task is really a coding or implementation task.\n"
            "If the job is about fixing or implementing software, prefer making code changes, validating them, and only write a document if the user explicitly asked for one.\n"
            "If the job is about research, summarization, or reporting, prefer producing a document or other readable file.\n"
            "Every job should converge toward a concrete deliverable, but that deliverable may be code rather than a report.\n"
            ...
            "If the user explicitly asked for a final document, only mark the job completed when that document is ready.\n"
            "If the user asked for code changes, only mark the job completed when the relevant code changes and essential validation are done.\n\n"
            f"Current job state:\n{job_json}\n\n"
            f"Recent job runs:\n{history_json}\n\n"
