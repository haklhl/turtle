...
- **应用层定时作业** — 定期检查结构化 `schedule.json`，支持脚本作业和 LLM 唤醒
- **Token 计费** — 自动统计用量和费用
...
Telegram 普通文本、图片、文件和带说明文字的附件消息都会转发给对应 Agent。入站附件会先下载到对应 agent workspace 下的 `.incoming/telegram/`，并按保留期自动清理。若 Agent 需要把本机已有图片或文件发回 Telegram，可在最终回复里输出一行 `ATTACH: /absolute/path/to/file`。`/model` 和 `/effort` 的切换会写回本地 `config.json`，因此重启后仍然生效。定时作业使用结构化 `schedule.json`，每次执行都会追加到 `schedule_runs.jsonl`，并向 owner 推送摘要。
